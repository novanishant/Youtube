...
const getAllVideos = asyncHandler(async (req, res) => {
  ...
  //   fetch only those videos where isPublished = true
  pipeline.push({ $match: { isPublished: true } });

  //sortBy can be on the basis of views, createdAt, duration
  //sortType can be in ascending(-1) or descending(1)
  if (sortBy && sortType) {
    pipeline.push({
      $sort: {
  ...
  return new ApiResponse(200, video, "Videos fetched successfully");
});
